...
public class LongestRepeatingSubseq {
    ...
    LongestRepeatingSubseq(int arr_len, int target) {
        size1 = arr_len;
        target1 = target;
    ...
    public int solve(List<String> s1, List<String> s2, int l1, int l2) {
        ...
        for (int i=1; i<(l1+1); i++){
            for(int j=1; j<(l2+1); j++){
                if(s1.get(i-1).equals(s2.get(j-1)) && i != j){
    ...
    public static void main(String[] args) {
        ...
        LongestRepeatingSubseq longestRepeatingSubseq = new LongestRepeatingSubseq(strings1.size()+1,strings2.size()+1 );
        int res = longestRepeatingSubseq.solve(strings1,strings2,str1.length(),str2.length());
        System.out.println("answer="+res);
        System.out.println("*****res1******");
        for (int i=0;i< strings1.size()+1;i++) {
            System.out.println();
            for (int j = 0; j < strings2.size()+1; j++) {
                System.out.print(longestRepeatingSubseq.res1[i][j]+",");
            }
        }
    }
}
